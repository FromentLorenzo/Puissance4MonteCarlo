...
class QLearningAgent:
    def __init__(self, alpha=0.1, gamma=0.9, epsilon=0.1, num_games=10000):
    ...
    def choose_action(self, state):
        ...
        if state_key not in self.q_table:
            return random.choice(legal_moves)  # If state is new, choose randomly
        q_values = [self.q_table[state_key].get(str(col), 0) for col in legal_moves]
    ...
    def train(self):
        ...
        for game_number in range(self.num_games):
            ...
            while not game_over:
                ...
                action = self.choose_action(state)  # Q-learning agent chooses an action
